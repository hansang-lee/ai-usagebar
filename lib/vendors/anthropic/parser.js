...
// Kept to 2 decimals (not whole percent) so the display can show e.g. 56.24%.
...
function windowPlaceholders(m, prefix, win, pace, now) {
    if (win) {
        m.set(`${prefix}_pct`, Math.max(0, 100 - win.utilizationPct).toFixed(2));
        m.set(`${prefix}_used_pct`, Number(win.utilizationPct).toFixed(2));
        m.set(`${prefix}_left_pct`, Math.max(0, 100 - win.utilizationPct).toFixed(2));
        m.set(`${prefix}_reset`, formatCountdown(win.resetsAt, now));
        m.set(`${prefix}_elapsed`, String(pace.elapsedPct));
    } else {
        m.set(`${prefix}_pct`, '100.00');
        m.set(`${prefix}_used_pct`, '0.00');
        m.set(`${prefix}_left_pct`, '100.00');
        m.set(`${prefix}_reset`, '—');
...
export function placeholders(snapshot, now) {
    ...
    const sessionResetsAt = snapshot.session?.resetsAt ?? new Date(now.getTime() + SESSION_MS);
    const sessionWin = snapshot.session ? {...snapshot.session, resetsAt: sessionResetsAt} : null;
    const sessionPace = calc({
        usagePct: sessionWin?.utilizationPct ?? 0,
        reset: sessionResetsAt,
        now,
        windowMs: SESSION_MS,
    });
    windowPlaceholders(m, 'session', sessionWin, sessionPace, now);

    const weeklyResetsAt = snapshot.weekly?.resetsAt ?? new Date(now.getTime() + WEEKLY_MS);
    const weeklyWin = snapshot.weekly ? {...snapshot.weekly, resetsAt: weeklyResetsAt} : null;
    const weeklyPace = calc({
        usagePct: weeklyWin?.utilizationPct ?? 0,
        reset: weeklyResetsAt,
        now,
        windowMs: WEEKLY_MS,
    });
    windowPlaceholders(m, 'weekly', weeklyWin, weeklyPace, now);
