...
export function buildSection(snapshot, meta, now, theme, _ = (s) => s) {
    ...
    const sessionWin = snapshot.session ? {
        ...snapshot.session,
        resetsAt: snapshot.session.resetsAt ?? new Date(now.getTime() + SESSION_MS),
    } : null;
    if (sessionWin)
        rows.push(windowRow(ICON_SESSION, _('Session'), sessionWin, SESSION_MS, now, theme, _));

    const weeklyWin = snapshot.weekly ? {
        ...snapshot.weekly,
        resetsAt: snapshot.weekly.resetsAt ?? new Date(now.getTime() + WEEKLY_MS),
    } : null;
    if (weeklyWin)
        rows.push(windowRow(ICON_WEEKLY, _('Weekly'), weeklyWin, WEEKLY_MS, now, theme, _));
    if (snapshot.sonnet)
