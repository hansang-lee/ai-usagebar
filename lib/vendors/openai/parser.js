...
// Kept to 2 decimals so the display matches the other vendors.
function pctLenient(v) {
...
function toWindow(w, defaultMs, now) {
    ...
    const pct = Math.min(100, Math.max(0, pctLenient(w.used_percent)));
...
function fmtLeftPct(v) {
    return Math.max(0, 100 - Number(v)).toFixed(2);
}
...
export function placeholders(snapshot, now) {
    ...
    m.set('session_pct', fmtLeftPct(session.utilizationPct));
    m.set('session_used_pct', fmtPct(session.utilizationPct));
    m.set('session_left_pct', fmtLeftPct(session.utilizationPct));
    m.set('session_reset', sessionReset);
    m.set('weekly_pct', fmtLeftPct(weekly.utilizationPct));
    m.set('weekly_used_pct', fmtPct(weekly.utilizationPct));
    m.set('weekly_left_pct', fmtLeftPct(weekly.utilizationPct));
    m.set('weekly_reset', weeklyReset);
    m.set('plan', snapshot.plan);

    m.set('oai_plan', snapshot.plan);
    m.set('oai_session_pct', fmtLeftPct(session.utilizationPct));
    ...
    m.set('oai_weekly_pct', fmtLeftPct(weekly.utilizationPct));
    ...
    m.set('oai_code_review_pct', fmtLeftPct(codeReview ? codeReview.utilizationPct : 0));
