...
export function parseAgyUsageOutput(raw) {
    ...
    for (const b of geminiGroup.buckets) {
        ...
        const utilizationPct = Math.min(100, Math.max(0, Number(((1 - remainingFraction) * 100).toFixed(2))));
...
async function doFetch(deps) {
    ...
    const agyQuota = agyPath
        ? await tryFetchAgyQuota(deps.runner ?? defaultAgyRunner, deps.signal, agyPath)
        : null;
