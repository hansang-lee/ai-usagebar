...
export function buildSection(snapshot, meta, now, theme, _ = (s) => s) {
    const rows = [];
    const usedPct = consumedPct(snapshot);
    ...
    rows.push({
        ...
        subLine: vformat(_('%s of %s used (%s%%)'),
            formatMoney(snapshot.totalUsage), formatMoney(snapshot.totalCredits), usedPct.toFixed(2)),
        color: severityColor(severityFor(usedPct), theme),
    });
