...
export function consumedPct(snap) {
    if (snap.totalCredits <= 0)
        return 0;
    return Math.min(100, Math.max(0, Math.round((snap.totalUsage / snap.totalCredits) * 10000) / 100));
}
...
export function placeholders(snap, _now) {
    const m = new Map();
    const used = consumedPct(snap);
    const pct = used.toFixed(2);
    const left = Math.max(0, 100 - used).toFixed(2);

    m.set('icon', ICON);
    m.set('vendor_short', VENDOR_SHORT);
    m.set('session_pct', left);
    m.set('session_used_pct', pct);
    m.set('session_left_pct', left);
    m.set('session_reset', '—');
    m.set('weekly_pct', left);
    m.set('weekly_used_pct', pct);
    m.set('weekly_left_pct', left);
    m.set('weekly_reset', '—');
