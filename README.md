...
- **Panel label format** (`bar-format`) — a template with `{token}` placeholders,
  ...
  `{session_pct}` / `{weekly_pct}` as the **remaining** share of the window
  (the `*_left_pct` aliases are identical); use `{session_used_pct}` /
  `{weekly_used_pct}` for how much has been consumed. The active vendor's
